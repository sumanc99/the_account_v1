...
if(isset($_POST["delete"])){
    ...
    if(!array_filter($errors)){
        $sql="UPDATE users SET status='active' WHERE phonenumber='$phone'";
        $sql_query = mysqli_query($conn, $sql);

        if(!$sql_query){
            echo"error".mysqli_error($conn);
        }else{
            $sms='staff is un-block successfully';
            $phone='';
        }
    }
                ...
                <div class="card-content">
                    <div class="input-field" style=" margin-bottom:0px !important;">
                        ...
                        <div class="right-align">
                             ...
                             <button name="delete"class="btn indigo darken-3 btn-brand z-depth-0">
                               un-block staff
                             </button>   
                        </div>
                    </div>
