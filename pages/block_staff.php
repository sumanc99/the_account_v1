...
if(isset($_POST["block"])){
    ...
    if(!array_filter($errors)){
        $sql="UPDATE users SET status='block' WHERE phonenumber='$phone'";
        $sql_query = mysqli_query($conn, $sql);

        if(!$sql_query){
            echo"error".mysqli_error($conn);
        }else{
            $sms='staff is block successfully';
            $phone='';
        }
    }
                ...
                <div class="card-content">
                    <div class="input-field" style=" margin-bottom:0px !important;">
                        ...
                        <div class="right-align">
                             ...
                             <button name="block"class="btn red darken-3 btn-brand-danger z-depth-0">
                                block staff
                             </button>   
                        </div>
                    </div>
